...
resource "aws_ami_from_instance" "roboshop-dev-component" {
  name = "${local.name}-${var.tags.Component}-ami-${local.current_time}"
...
resource "null_resource" "delete_instance" {
    ...
    depends_on = [ aws_ami_from_instance.roboshop-dev-component ]
